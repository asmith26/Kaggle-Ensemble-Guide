...
with open(loc_outfile,"wb") as outfile:
  ...
  #        print "{0}, {1}".format(file_idx, line_idx)
          ...
          if file_idx == 0 and line_idx > 0:
              combined_row_predictions[row_predictions[0]] = [ 0 for i in range(len(row_predictions[1:])) ]

          if line_idx > 0:
              current_total = combined_row_predictions[row_predictions[0]]
              combined_row_predictions[row_predictions[0]] = [a+float(b) for a,b in zip(current_total, row_predictions[1:])]
              
    for k,v in combined_row_predictions.iteritems():
        csv_predictions = ",".join(map(str, [prediction /(file_idx+1) for prediction in combined_row_predictions[k]]))
